...
// sono stati definiti ai numeri originali i nueri calcolati da matlab partendo daalle istruzioni presenti sulla consegna
#define N 10 /// order of the filter
#define NT (N+1) /// number of coeffs
#define NB 11 /// number of bits
#define SHAMT 13 /// shift amount
//shamt è stato impostato dopo tentativi ricorsivi su matlab per minimizzare il thd;

const int bi[NT]={-1,	-13,	-26,	65,	281,	407,	281,	65,	-26,	-13,	-1}; /// b array 
...

{
  ...
  if (argc != 3)
  {
    printf("Use: %s samples.txt resultsc.txt\n", argv[0]);
    exit(1);
  }
